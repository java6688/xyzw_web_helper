...
export const useTokenStore = defineStore("tokens", () => {
  ...
  const createWebSocketConnection = async (
    ...
    if (activeConnectionCount.value < maxConcurrentConnections.value) {
      ...
      try {
        const client = await createWebSocketConnectionInternal(
          tokenId,
          base64Token,
          customWsUrl,
        );
  ...
  const processConnectionQueue = async () => {
    ...
    if (
      activeConnectionCount.value >= maxConcurrentConnections.value ||
      connectionQueue.value.length === 0
    ) {
      return;
    }
    ...
    try {
      ...
      activeConnectionCount.value++;
      wsLogger.info(
        `开始处理队列中的连接: ${tokenId}，当前活跃连接数: ${activeConnectionCount.value}`,
      );

      // 创建连接
      await createWebSocketConnectionInternal(
        tokenId,
        token.token,
        token.wsUrl,
      );
      ...
      activeConnectionCount.value--;
      wsLogger.info(
        `队列连接处理完成: ${tokenId}，当前活跃连接数: ${activeConnectionCount.value}`,
      );
  ...
  const enqueueConnection = (tokenId: string) => {
    ...
    wsLogger.info(
      `Token added to connection queue: ${tokenId}, position: ${position + 1}, estimated wait: ${waitTime}ms`,
    );
